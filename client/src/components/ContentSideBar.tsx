...
const ContentBar: React.FC<State> = ({setBooleans, sideOpen}) => {
  ...
  const {logout, userProfile} = useContext(contextContent);
    const redirect = useNavigate();
    
    console.log('Tangina: ', userProfile.id);
   
  return (
    <div>
      <div className=" md:hidden absolute top-[2rem]">
        <Menu 
        onClick={()=> setBooleans(pro => ({...pro, sideOpen:!pro.sideOpen}))}
        size={40} className="cursor-pointer"/>
      </div>
     <aside className={`min-h-[100vh] w-[17rem] shadow-2xl  md:block ${sideOpen? 'block': 'hidden'} z-50`}>
      ...
      <div className="h-[15rem]  flex justify-center items-center p-5">
        <div className="w-[100%] h-[15rem] flex justify-center items-center  shadow-2xl">
          <p className="text-5xl">{userProfile? userProfile.name.charAt(0): 'Loaading.........'}</p>
        </div>
      </div>
